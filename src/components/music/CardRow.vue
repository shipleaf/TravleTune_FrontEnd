<template>
  <div class="boxes">
    ...
    <div
      class="box"
      style="
        --src: url(https://www.udiscovermusic.com/wp-content/uploads/2015/10/Kanye-West-Yeezus.jpg);
      "
    >
      <span>10</span
...
onMounted(() => {
  ...
  const dragProxyEl = document.querySelector('.drag-proxy')
  const nextBtn = document.querySelector('.next')
  const prevBtn = document.querySelector('.prev')

  if (!boxesEl || !dragProxyEl || !nextBtn || !prevBtn) {
  ...
  onBeforeUnmount(() => {
    ...
    ScrollTrigger.killAll(false) // 이 컴포넌트 관련 트리거 정리
  })
})
</script>
